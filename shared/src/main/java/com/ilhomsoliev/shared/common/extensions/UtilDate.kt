...
import com.ilhomsoliev.shared.common.extensions.DayOfWeek.Companion.display
import com.ilhomsoliev.shared.common.extensions.Month.Companion.display
import com.ilhomsoliev.shared.common.extensions.Month.Companion.displayRodName
...
fun getDateInYearMonthDay(year: Int, month: java.time.Month, day: Int) =
    "$year-${month.value}-$day" // yyyy-mm-dd

@RequiresApi(Build.VERSION_CODES.O)
fun getDateInYearMonth(year: Int, month: java.time.Month) = "$year-${month.value}" // yyyy-mm-dd

fun getDateString(date: LocalDate, isNameEnabled: Boolean = false): String { // 15 марта
    ...
    return "${if (isNameEnabled) "$wdName " else ""}${date.day()} $month"
}
